...
import { EXAM_CONFIG } from "@/lib/rbi-grade-b/config";

describe("RBI Grade B Config", () => {
  it("has correct exam ID", () => {
    expect(EXAM_CONFIG.examId).toBe("EXAM054");
  });

  it("has correct slug", () => {
    expect(EXAM_CONFIG.slug).toBe("rbi-grade-b");
  });
